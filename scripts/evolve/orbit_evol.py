...
def evolve_orbit(Star, q_init, a_init, e_init, EvPars, verbosity=1, 
                 start_at_zams=True, solve_method='Radau'):
    """
    ...
    if not hasattr(EvPars, 'zams_spin_factor'): EvPars.zams_spin_factor = 0.3
    if not hasattr(EvPars, 'tide_model'): EvPars.tide_model = 'BSE'
    if not hasattr(EvPars, 'wind_model'): EvPars.wind_model = 'BHL'
    if not hasattr(EvPars, 'MB_scale_factor'): EvPars.MB_scale_factor = 1.0
    if not hasattr(EvPars, 'tide_scale_factor'): EvPars.tide_scale_factor = 1.0
    if not hasattr(EvPars, 'tidal_freq_scaling'): EvPars.tidal_freq_scaling = 'BSE'
    if not hasattr(EvPars, 'tide_max_fconv'): EvPars.tide_max_fconv = 1e6
    ...
    if start_at_zams:
        ...
        spin = spin * (Star.radius[nzams]/Star.radius[0])**2\
                    * Star.rg2[nzams]/Star.rg2[0]
    ...
    if verbosity > 1: print( f'initial spin period: {2*np.pi/spin/const.day:.6f} d' )
    ...
    while (Bin.status == 'detached') & (i < len(dM)):
        ...
        dln_a_dt, de_dt, dspin_dt, dmcomp_dt, dj_dt = orbit_evol_eqs(sma, ecc, spin,
                                           mcomp, SP, EvPars)
        ...
        log_RL_next = np.log10(roche_lobe(Star.mass[i+1]/mcomp_next) * sma_next *
                               (1 - ecc_next))
        if Star.log_R[i+1] > log_RL_next: 
            slowly_changing = False
            if verbosity > 1: print(
                    f'\n[!] RL overfilled at model {i+1} by '
					f'd(log R) = {Star.log_R[i+1] - log_RL_next:.6f}', end='' )

        if (verbosity == 1) & (i % 50 == 0): print( '.', end='', flush=True )
        if (verbosity > 1) & (i % 20 == 0): print( 
        ...
        if slowly_changing:
            ...
            sol = solve_ivp( derivs, t_interval, y0, events=fill_RL, \
                    method=solve_method, \
            ...
            if len(sol.t_events[0]) > 0: Bin.status = "RLOF"
    ...
    if verbosity: 
        ...
        print( f'steps taken: {len(track_int_simple)} by simple intergration, '
               f'{len(track_int_solve)} ({solve_steps}) using solve_ivp' )

    # Check how well angular momentum is conserved.
    am_orb = (Bin.m1*Bin.m2)/(Bin.m1 + Bin.m2) * Bin.a**2 * \
            omega_Kep(Bin.m1 + Bin.m2, Bin.a) * np.sqrt(1 - Bin.e**2)
    ...
    if verbosity: print( f'AM conserved up to {100*am_cons:.4f}%' )
...
def derivs(t, y, M_func, logR_func, SP, EvPars):
    ...
    dln_a_dt, de_dt, dspin_dt, dmcomp_dt, dj_dt = orbit_evol_eqs(a, e, spin, mcomp,
            SP, EvPars)
...
def orbit_evol_eqs(a, e, spin, mcomp, SP, EvPars):
    ...
    dln_a_dt, de_dt, dspin_dt, dmcomp_dt, dj_dt = wind_loss(a, e, spin, mcomp, SP,
                                                     EvPars)
...
def wind_loss(a, e, spin, mcomp, SP, EvPars):
    ...
    dln_a_dt = -2*SP.dM_dt/SP.mass * (1 - beta*Q - (1 - beta)*(eta*(1 + Q)
                + 0.5*Q/(1 + Q))) + 2*e/(1 - e**2) * de_dt
    ...
    dj_dt_orb = (1 - beta)*eta * a**2 * np.sqrt(1 - e**2) * omega_Kep(SP.mass + 
                mcomp, a) * SP.dM_dt
...
def tides(a, e, spin, mcomp, SP, EvPars):
    ...
    de_dt = 0
    if e > 1e-7: de_dt = -27*tide_factor * e * (ge[3] - (11./18)*ge[4]*spin/o_orb)

    # Change in spin frequency, Hut's eq. (11)
    dspin_dt = 3*k_over_T * q**2/SP.rg2 * (R_over_a)**6 * (ge[2]*o_orb - ge[5]*spin)
...
def dissipation_rate(a, q, spin, o_orb, SP, EvPars):
    ...
    if EvPars.tide_model == 'Preece':
        ...
        k_over_T_conv = (2./21.) * (SP.m_cenv/SP.mass) / SP.tconv
    else:
        k_over_T_conv = 0
    ...
    if SP.m_cenv < 1e-2:
        ...
        k_over_T_rad = 1.98e4 * E2 * np.sqrt(SP.mass/SP.radius**3)\
                       * (1 + q)**(5.0/6.0) * (SP.radius/a)**2.5
    else:
        k_over_T_rad = 0
