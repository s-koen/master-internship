...
def read_stellar_models (masses, mist_dir):
    """
    Read MIST stellar evolution tracks for a range of masses, and store as a list of 
    StellarModel objects.

    Args:
        masses: stellar masses [Msun] for which data is to be read.
            if a tuple: read all files with masses in the interval (masses[0], masses[1]).
            if a list: read files for the given mass values. 
        mist_dir: pathname of MIST directory (string).
    """
    if type(masses) == list:
        # read files for masses in list
        mstring = [f'{int(100*m):05}' for m in masses]
        tracks = [read_mist_models.EEP( mist_dir + m + 'M.track.eep' ) for m in mstring]
    elif type(masses) == tuple:
        # read all files in mass interval given by masses
        eep_files = glob.glob( mist_dir + '*.eep' )
        eep_files.sort()
        minit = np.array( [float(f[-16:-11])/100 for f in eep_files] )
        select = np.where( (minit >= masses[0]) & (minit <= masses[1]) )[0]
        sel_files = [eep_files[i] for i in select]
        tracks = [read_mist_models.EEP( f ) for f in sel_files]
    else:
        print( 'Illegal type for masses, must be list or 2-tuple.' )
        return None

    print( 'read', len(tracks), 'files' )
    Stars = [StellarModel(tr) for tr in tracks]
    return Stars
...
class StellarModel:
    ...
    nzams, ntams, nrgbt, nzacheb, ntacheb, ntpagb, npagb, nwd = \
        201, 453, 604, 630, 706, 807, 1408, 1709
    ...
    def __init__(self, track):
        ...
        self.m_cenv, self.r_cenv = \
            conv_envelope_size( self.mass, self.m_core, self.radius, self.r_core, \
                                self.lum, self.log_Teff, self.age, self.model_TAMS, \
                                self.model_BGB, self.model_Rmax_HSB )
        self.tconv = \
            conv_turnover_time( self.m_cenv, self.r_cenv, self.radius, self.lum )
        # gyration radius; k2g and k2r below are not needed but useful for testing
        self.rg2, self.k2g, self.k2r = \
            gyration_radius(self.mass, self.m_core, self.radius, self.r_core, \
                            self.lum, self.log_Teff, self.age, self.Z_init)
        # viscous tidal dissipation rate for convective envelopes
        self.k_over_T_conv = \
            conv_dissipation_rate(self.m_cenv, self.r_cenv, self.mass, self.radius,
                                  self.lum, self.Z_init)
        ...
        self.HPgas = gas_pressure_scale_height( self.mass, self.radius, self.log_Teff,
                                                mmw_surf )
...
#        return mod_Rmax_HSB
    ...
    def find_first_TP (self):
        if (self.n_models > ntpagb+1): 
            dblsh = np.where((self.age >= self.age[ntpagb]) & \
                    (self.log_LH > self.log_LHe + 0.5))[0]
            if len(dblsh) > 0:
                mod_dblsh = dblsh[0]
                tpone = np.where((self.age >= self.age[mod_dblsh]) & \
                        (self.log_LHe > self.log_LHe[mod_dblsh] + 0.2))[0]
                if len(tpone) > 0:
                    mod_TPone = tpone[0] - 1
                else:
...
def gas_pressure_scale_height (mass, radius, log_Teff, mmw):
    ...
    """
    H_P = (const.R_gas*10**log_Teff/mmw) * (radius*const.Rsun)**2\
...
def conv_turnover_time (m_env, r_env, radius, lum):
    ...
    """
    t_conv_yr = 0.4311 * (m_env*r_env*(radius - 0.5*r_env)/(3*lum))**(1./3.)
    return t_conv_yr
...
def conv_dissipation_rate (m_cenv, r_cenv, mass, radius, lum, Z):
    ...
    # Preece et al (2022).
    t_conv = 0.4311 * (3*m_cenv * r_cenv**2 / lum)**(1./3.)
...
def Teff_at_BGB (mass, Z):
    ...
    Teff_bgb = ((L_bgb*const.Lsun) / (4*np.pi*const.sigma_SB*(R_bgb*const.Rsun)**2))**0.25
    return Teff_bgb
...
def gyration_radius (mass, m_core, radius, r_core, lum, log_Teff, age, Z):
    ...
    if log_m0 > 1.3: k2z = k2z - 0.055*(log_m0 - 1.3)**2
    ...
    B = 1.0e4*mass**(3./2.)/(1.0 + 0.1*mass**(3./2.))
    x = ((lum - L_bgb)/B)**2
    ...
    rg2 = k2e*((mass - m_core)/mass) + k2c*(m_core/mass)*(r_core/radius)**2
