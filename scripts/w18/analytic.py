...
for R, q, model in grid_min.iter_models():
    ...
    plt.scatter(
        R,
        q,
        c=np.max(model.star.R / model.star.binary_separation),
        cmap="bwr",
        vmin=np.min(Z),
        vmax=1 + (1 - np.min(Z)),
    )
...
plt.ylabel("$q$")
plt.savefig(
    "/home/koen/LaTeX-setup/plots/w18-compare-mesa-analytic.pgf",
    format="pgf",
    dpi=300,
)
plt.show()
...
fig.write_html(
    "/home/koen/figures/plots/master-internship/w18/fit-surface-low.html",
    include_plotlyjs="cdn",
    include_mathjax="cdn",
    config={"responsive": True},
)
...
plt.title(r"$\delta=0.5,\;\gamma = 1.0$")
plt.xlabel("Roche lobe radius ($R_\odot$)")
plt.ylabel("$q$")
plt.savefig(
    "/home/koen/LaTeX-setup/plots/w18-compare-mesa-analytic-high.pgf",
    format="pgf",
    dpi=300,
)
plt.show()
...
fig.update_layout(
    ...
    autosize=True,
    margin=dict(l=0, r=0, b=0, t=50),
    title="With a radius fit using the grid of models with delta=0.5, gamma=1.0",
)
