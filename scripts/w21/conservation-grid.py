...
for i, model in enumerate(true_list):
    ...
    model_labels.append(f"delta = {deltas[i]:.1f}, beta = {1-deltas[i]:.1f}")
...
fig.write_html(
    "/home/koen/figures/plots/master-internship/w21/delta-grid-grav-luminosity.html",
...
for i, m in enumerate(models2):
    plt.plot(m.star_1_mass, m.star_2_mass, c=colors[i])
...
axs.set_facecolor("C9")
plt.xlabel(r"$M_\textrm{d}$ ($M_\odot$)")
plt.ylabel(r"$M_\textrm{a}$ ($M_\odot$)")
plt.savefig("/home/koen/LaTeX-setup/plots/w21-mdma.pgf", format="pgf")
plt.show()
plt.close()
...
def compute_lgrav(profile):
    ...
    m = profile.mass[::-1]
    r = profile.mass[0] - profile.mass
    # eps = profile.eps_grav[::-1][1:]

    dm = np.diff(m)

    lgrav = profile.c12[::-1] / profile.o16[::-1] * 16 / 12
...
for i, model in enumerate(true_list):
    ...
    model_t.append(t_series)
    model_lum.append(lum_series)
    model_labels.append(f"delta = {i:.1f}, beta = {1-i:.1f}")
...
for i in range(n_models):
    ...
    l0_interp = np.interp(R_grid, r0, l0[::-1], left=np.nan, right=np.nan)
...
for t in range(n_steps):
    ...
    for i in range(n_models):
        ...
        l_interp = np.interp(R_grid, r, l[::-1], left=np.nan, right=np.nan)
...
for profile in true_list[0]:
    plt.plot(profile.mass, profile.c12 / profile.o16 * 16 / 12)
plt.show()
# %%
