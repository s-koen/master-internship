...
# %%
deltat = 1.2474427e9 + 83 - 94400 + 356600
...
plt.ylim(150, 900)
plt.xlim(0, 1000)
plt.vlines(494.7, 0, 10000)
plt.vlines(754.4, 0, 10000)
# plt.yticks([150,450,750])
plt.xlabel("Time (yr)")
plt.ylabel("Radius ($R_\\odot$)")
# plt.savefig("results5.svg", format="svg")
plt.show()
plt.close()
...
for i, (R, q, model) in enumerate(grid.get_R1_index(3)):
    ...
    stop = np.argwhere(bin.age > model.age[0] + grid.tpagb_age)[0][0]
    plt.plot(bin.age[:stop] - time - delta, orbital_period_days(bin)[:stop])
...
plt.xlim(-1e6, 1e6)
plt.ylim(10 ** (1.5), 10**4.8333333)
plt.savefig("results6.svg", format="svg", dpi=600)
plt.show()
...
plt.colorbar(
    mesh,
    label=r"Orbital period (days)",
    ticks=[np.log10(0.7), np.log10(0.85), 0, np.log10(1.25), np.log10(1.5)],
)
...
colors = ["#fcd4e8", "#ec248c", "#0867ce", "#a9cdef"]
...
mesh = ax.pcolormesh(
    R_edges, q_edges, Z, cmap=cmr.torch, shading="auto", rasterized=True
)
...
plt.savefig("results7.svg", format="svg", dpi=600)
# plt.savefig("/home/koen/LaTeX-setup/plots/w12-grid-4.pgf", format="pgf")
plt.show()
plt.close()
...
for i, (R, q, model) in enumerate(grid.get_R1_index(-1)):
    ...
    axs.plot(Star.age - deltat, np.log10(Star.Mdot), c=f"C{i}", alpha=0.4, linewidth=3)
...
plt.xlabel("Time (yr)")
plt.ylabel("Radius ($R_\\odot$)")
plt.savefig("results10.svg", format="svg")
plt.show()
plt.close()
...
plt.xlabel("Time (yr)")
plt.ylabel("Radius ($R_\\odot$)")
plt.savefig("results12.svg", format="svg")
plt.show()
plt.close()
