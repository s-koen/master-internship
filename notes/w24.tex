...
Because the \(M=2.2\), \(2.6\) and \(3.0\;M_\odot \) models contain very few finished models, it does not make much sense to look at these grids in more detail. For the \(M=1.8\;M_\odot \) model, we can look at the grid.

\begin{figure}[ht]
    \centering
    \incplot{w24-grid-period-days}
\end{figure}

The right most column has quite significantly larger periods than the other models. This is because these models miss RLOF:

\begin{figure}[ht]
    \centering
    \incplot{w24-miss-rlof}
\end{figure}
...
I also wrote a routine to compute combine the binary MESA evolution with the single star evolution to continue the DUP evolution.

Below are examples:
\begin{figure}[ht]
    \centering
    \incplot{w24-dup-combined-age}
\end{figure}

\begin{figure}[ht]
    \centering
    \incplot{w24-dup-combined-m_env}
\end{figure}
...
Unfortunately, this also does not fix every binary model. On the model that I tried, I managed to evolve a bit further, but it ultimately ended in with the same convergence problems.

What I have found that does seem to work if the convergence problems are persistent, is to force the star to remesh with a significantly different number of zones close to the convergence problems. There is then a big chance that this remeshing skips over the original convergence problems.

The only way that I can think to automate this, is by wrapping the whole \emph{MESA} run in another script that detects the convergence problems, changes the mesh coefficient temporarily and then resumes the \emph{MESA} simulation from the last photo.

I think it is worth trying out, and I think it should not be too difficult. Especially if we use the \lstinline[style=output, breaklines=true]|resolve_mesh_during_mass_loss| formalism from above.

To give an idea, by manually doing this method, I was able to simulate \emph{all} single stars, \emph{and} for the binary models that were stuck, I was able to revive them with only one additional resume call after the initial resume.
